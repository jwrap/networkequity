...
def gen_prod_attr(district_stats, prod_driver, attr_driver='Population_x', mult=False):
    '''
...
def district_stats_to_OD_df(gdf_points, prod_driver, attr_driver='Population_x', deterrence=dist_deterrence, mult=False):
    ...
    production, attraction = gen_prod_attr(gdf_points, prod_driver, attr_driver, mult=mult)
...
def all_ods_creation_ema(gdf_points, prod_lists,attr_driver, dist_deterrence=dist_deterrence):

    od_dict={}
    for prod in prod_lists:
        od = district_stats_to_OD_df(gdf_points,  prod_driver=prod, attr_driver=attr_driver, deterrence=dist_deterrence, mult=True)
